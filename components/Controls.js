...
function reducer(state, action) {
  switch (action.type) {
    case "pushUser":
      ...
      copyListUsers[action.userName]={userName:action.userName,timeAdd: new Date().getTime()};
      ...
      <form className="controls" onSubmit={handleSubmit}>
        <div className="controls-wrap">
          {Object.keys(state.listUsers).length ? (
            <div className="controls-typing">{Object.keys(state.listUsers).join(", ")} is writting...</div>
          ) : null}
